    ...
    def load_ctrlregen_ip_adapter(
        ...
        **kwargs,
        ...
        if len(weight_name) != len(pretrained_model_name_or_path_or_dict):
            raise ValueError(
                "`weight_name` and `pretrained_model_name_or_path_or_dict` "
                "must have the same length."
            )
        if len(weight_name) != len(subfolder):
            raise ValueError(
                "`weight_name` and `subfolder` must have the same length."
            )
        ...
        for path_or_dict, wn, sf in zip(
            ...
            if keys != ["image_proj", "ip_adapter"]:
                raise ValueError(
                    "Required keys (`image_proj` and `ip_adapter`) "
        ...
        if hasattr(self, "image_encoder") and getattr(self, "image_encoder", None) is None:
            if image_encoder_folder is not None:
                ...
                image_encoder = AutoModel.from_pretrained(DINOV2_MODEL_ID).to(
                    self.device, dtype=enc_dtype  # type: ignore[attr-defined]
                )
